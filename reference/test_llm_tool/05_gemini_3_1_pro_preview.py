...
SCRIPT_ID = "05_gemini_3_1_pro_preview"
PROFILE_MATCHERS = ["GG"]
...
HTTP_RETRY_DELAYS_SECONDS = [2, 4, 8]
...
def run_probe(
    *,
    ...
    try:
        ...
        fallback_model = rate_limit_fallback_model(model)
        ...
        try:
            result = run_probe_once(
                ...
                prompt=default_prompt(fallback_model) if prompt == default_prompt(model) else prompt,
                mode=mode,
                max_rounds=max_rounds,
                auth_token=auth_token,
            )
            ...
            raise SystemExit(
                f"{exc}\nFallback model also rate limited: {fallback_model}\n{fallback_exc}"
            ) from fallback_exc
        result["requestedModel"] = model
        result["rateLimitFallbackModel"] = fallback_model
        result["rateLimitFallbackReason"] = str(exc)
        return result
...
def build_json_prompt(prompt: str, model: str) -> str:
    ...
    return (
        ...
        + '{"final":true,"reply":"涓枃涓€鍙ヨ瘽鎬荤粨","used_tools":["..."],"preview_path":"sample/test_llm_tool/_probe_output.txt"}. '
        + "No markdown fences.\n"
...
def call_openai_native(
    *,
    ...
    response = post_json_with_retries(
        url=chat_url(str(profile.get("baseUrl") or "")),
        headers=build_headers(profile, auth_token=auth_token, json_content=True),
        body={
            "model": model,
            "messages": messages,
...
def parse_response_json(response: httpx.Response, *, native: bool) -> Dict[str, Any]:
    ...
    if response.status_code >= 400:
        ...
        if response.status_code == 429:
            raise ProviderRateLimitError(f"Provider rate limited the request after retries: {text}")
        if native and looks_like_tool_unsupported(response.status_code, lowered):
...
def post_json_with_retries(*, url: str, headers: Dict[str, str], body: Dict[str, Any]) -> httpx.Response:
    last_response: Optional[httpx.Response] = None
    for attempt_index in range(len(HTTP_RETRY_DELAYS_SECONDS) + 1):
        response = httpx.post(url, headers=headers, json=body, timeout=120)
        last_response = response
        if response.status_code != 429:
            return response
        if attempt_index >= len(HTTP_RETRY_DELAYS_SECONDS):
            break
        time.sleep(HTTP_RETRY_DELAYS_SECONDS[attempt_index])
    if last_response is None:
        raise SystemExit("Request was not sent.")
    return last_response


def looks_like_tool_unsupported(status_code: int, lowered_text: str) -> bool:
    if status_code not in {400, 404, 422}:
        return False
    markers = [
        "does not support this tool",
...
def rate_limit_fallback_model(model: str) -> str:
    normalized = str(model or "").strip()
    if normalized == "gemini-3.1-pro-preview":
        return "gemini-3.1-pro-preview-low"
    return ""
